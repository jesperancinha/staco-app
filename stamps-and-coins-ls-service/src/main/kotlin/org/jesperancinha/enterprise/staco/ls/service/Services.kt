...
internal class StacoDao(
    ...
    fun saveCoin(staCoDto: StaCoDto): Mono<StaCoDto> = staCoRepository.save(staCoDto.toEvent)
        .map { it.attributes().toDto.also { logger.debug { "Coin $staCoDto saved in dynamoDb" } } }

    fun saveStamp(staCoDto: StaCoDto): Mono<StaCoDto> = staCoRepository.save(staCoDto.toEvent)
